...
exports.deleteAll = (req, res) => {
    Enable.removeAll((err, data) => {
        if (err)
            res.status(500).send({
                message:
                    err.message || "Some error occurred while removing all enablenames."
            });
        else res.send({ message: `All Enables were deleted successfully!` });
    });
};
