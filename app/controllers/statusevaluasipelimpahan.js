...
exports.findAll = (req, res) => {
    StatusEvaluasiPelimpahan.getAll((err, data) => {
        if (err)
            res.status(500).send({
                message:
...
exports.deleteAll = (req, res) => {
    StatusEvaluasiPelimpahan.removeAll((err, data) => {
        if (err)
            res.status(500).send({
                message:
                    err.message || "Some error occurred while removing all statusevaluasipelimpahannames."
            });
        else res.send({ message: `All StatusEvaluasiPelimpahans were deleted successfully!` });
    });
};
