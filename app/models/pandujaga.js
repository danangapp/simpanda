...
PanduJaga.findById = async (id, result) => {
	...
	sql.query(`SELECT a.* , a1.*, a2.* FROM pandu_jaga a  LEFT JOIN pandu_schedule a1 ON a.pandu_schedule_id = a1.id  LEFT JOIN personil a2 ON a.personil_id = a2.id  WHERE a.id = ${id}`, (err, res) => {
		if (err) {
