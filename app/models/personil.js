...
Personil.findById = async (id, result) => {
    ...
    sql.query(`SELECT a.* , a1.flag as flag, a2.nama as approval_status, a3.nama as ena, a4.nama_asset as asset_kapal, a5.nama as status_kepegawaian, a6.nama as tempat_tu FROM personil a  LEFT JOIN tipe_personil a1 ON a.tipe_personil_id = a1.id  LEFT JOIN approval_status a2 ON a.approval_status_id = a2.id  LEFT JOIN enable a3 ON a.enable = a3.id  LEFT JOIN asset_kapal a4 ON a.asset_kapal_id = a4.id  LEFT JOIN status_kepegawaian a5 ON a.status_kepegawaian_id = a5.id  LEFT JOIN cabang a6 ON a.tempat_tugas = a6.id  WHERE a.id = ${id}`, (err, res) => {
        if (err) {
...
Personil.updateById = async(id, personil, result) => {
	try {
		...
		await query("DELETE FROM sertifikat WHERE personil_id='" + id + "'");
		for (var i in sertifikat) {
		    const x = sertifikat[i];
